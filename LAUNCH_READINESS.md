...
|---|---|---|---|
| 1 | Root-relative internal links (404s on `/modelsignal/`) | ✅ FIXED & VERIFIED LIVE | Single helper `u()` in `pipeline/ssg.py`; every anchor in generator goes through it; `pipeline/tests/test_links.py` (4 tests) fails on any root-relative or dangling link; production crawl following real anchors visited **29 pages, all HTTP 200, 0 broken** (log in this file's appendix) |
